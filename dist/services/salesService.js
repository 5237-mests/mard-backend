...
class SalesService {
    ...
    static getSalesPaginated(params) {
        return __awaiter(this, void 0, void 0, function* () {
            const { shopId, startDate, endDate, status, search, page = 1, limit = 20, sortBy = "created_at", sortOrder = "desc", } = params;
            let where = [];
            let queryParams = [];
            if (shopId) {
            ...
            if (search) {
                where.push("(i.name LIKE ? OR i.model LIKE ? OR s.customer_name LIKE ? OR s.customer_contact LIKE ?)");
                const likeSearch = `%${search}%`;
                queryParams.push(likeSearch, likeSearch, likeSearch, likeSearch);
            }
            const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
            // Total count
            const [countRows] = yield (0, db_1.query)(`SELECT COUNT(DISTINCT s.id) as total FROM sales s LEFT JOIN sale_items si ON s.id = si.sale_id LEFT JOIN items i ON si.item_id = i.id ${whereSql}`, queryParams);
      ...
      `, mainParams);
            ...
            const sales = salesRows.map((row) => (Object.assign(Object.assign({}, row), { items: JSON.parse(row.items) || [], total_items_sold: Number(row.total_items_sold) || 0, distinct_items_count: Number(row.distinct_items_count) || 0, final_amount: row.total_amount })));
            // Summary aggregates
            const [summaryRows] = yield (0, db_1.query)(`
      SELECT 
    ...
    static userBelongsToShop(userId, shopId) {
        return __awaiter(this, void 0, void 0, function* () {
            const [rows] = yield (0, db_1.query)("SELECT COUNT(*) as count FROM shop_shopkeepers WHERE user_id = ? AND shop_id = ?", [userId, shopId]);
    ...
    static refundSale(identifier, partialItems) {
        return __awaiter(this, void 0, void 0, function* () {
            const { saleId, tx_ref } = identifier;
            if (!saleId && !tx_ref)
                throw new Error("Provide saleId or tx_ref");
            yield (0, db_1.transaction)((connection) => __awaiter(this, void 0, void 0, function* () {
                const [sales] = yield connection.query("SELECT id, shop_id, status FROM sales WHERE ${saleId ? 'id = ?' : 'tx_ref = ?'}", [saleId || tx_ref]);
