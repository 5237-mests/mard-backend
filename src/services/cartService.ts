...
export const getAvailableStock = async (itemId: number) => {
...
export const addToCart = async (item: CartItem, user_id: number) => {
  return await transaction(async (conn) => {
    ...
    const availableStock = await getAvailableStock(item.item_id);
    if (availableStock < item.quantity) {
      throw new AppError(
        `Insufficient stock for item ${item.item_id}. Available: ${availableStock}, Requested: ${item.quantity}`,
        400,
      );
    }
...
export const incrementCartItem = async (userId: number, itemId: number) => {
  ...
  const availableStock = await getAvailableStock(itemId);
  if (availableStock <= 0) {
    throw new AppError("Item is out of stock", 404);
  }
  ...
  if (currentCartQty + 1 > availableStock) {
    throw new AppError(
      `Insufficient stock. Available: ${availableStock}, Requested: ${
        currentCartQty + 1
      }`,
      400,
    );
